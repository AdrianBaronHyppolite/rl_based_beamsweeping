...
class RLIAEnv(Env):
    ...
    def calc_path_loss_umi(self, fc_ghz, d_2d_meter, is_los):
        # Constants
        H_UT = 1.5  # Height of UT (m)
        H_BS = 10  # Height of BS (m)
        ...
        if is_los:
            ...
            pl = 32.4 + 20 * np.log10(d_3d) + 20 * np.log10(fc_ghz)
        else:
            # 3GPP UMi-Street Canyon NLoS Path Loss Model
            pl = 22.4 + 35.3 * np.log10(d_3d) + 21.3 * np.log10(fc_ghz) - 0.3 * (H_UT - 1.5)
    ...
    def sweep(self, txpat):
        ...
        for x in xs:
                for y in ys:
                    ...
                    for codebook_id in txpat:
                        ...
                        rss = self.ptx_dbm + g_dBi - self.calc_path_loss_umi(30, self.get_dis(self.ue_loc), True)
                        ...
                        everss = 30 + eveg_dBi - self.calc_path_loss_umi(30, self.get_dis([x,y]), True)
        ...
        #self.preid = 32
        print("This is my max dbi", int(maxdbiid[1]))
        self.preid = 32
        #int(maxdbiid[1])

        #get the id of the beam with the highest rssi
    ...
    def rewardfunction(self, action):
        rfunc = (self.alpha[4] * self.cbsp) - ((1-self.alpha[4]) * self.pp[action])
        return rfunc
    ...
    def step(self, action):
        ...
        self.state = actset[0], actset[2], actset[4], actset[6]
    ...
    def teststep(self, action):
        ...
        self.ue_loc[0] = 10*np.cos(np.deg2rad(self.step_count*10))
        self.ue_loc[1] = 10*np.sin(np.deg2rad(self.step_count*10))
